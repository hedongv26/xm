            ...
            <div slot="content" class="list-item-body-content">
              ...
              <div class="list-item-row">
                ...
                <div class="list-item-row-r can-show-more" @click="showProgressList(item.id)">
                  <svg-icon class="more-foreign" icon-class="more"></svg-icon>
                  <span style="color: darkgreen">{{item.progress}}</span>
                </div>
              </div>
              <div class="list-item-row">
                <div class="list-item-row-l">
                  <div class="justify-6">世界500强</div>
                </div>
              ...
              <div class="list-item-row">
                ...
                <div class="list-item-row-r"><span style="color: orange">{{item.investment}}</span>（万美元）</div>
              </div>
              <div class="list-item-row">
                <div class="list-item-row-l">
                  <div>{{titles.contractural_foreign_investment}}</div>
                </div>
                <div class="list-item-row-r"><span style="color: orange">{{item.contractural_foreign_investment}}</span>（万美元）
                </div>
              </div>
              <div class="list-item-row">
                <div class="list-item-row-l">
                ...
                <div class="list-item-row-r">{{item.land_area}}（平方米）</div>
              </div>
              <div class="list-item-row">
                <div class="list-item-row-l">
                  <div>{{titles.investment_mode}}</div>
                </div>
                <div class="list-item-row-r">{{item.investment_mode}}</div>
              </div>
              <div class="list-item-row">
                <div class="list-item-row-l">
              ...
              <div class="list-item-row">
                <div class="list-item-row-l">
                  <div class="justify-4">{{titles.foreign}}</div>
                </div>
                <div class="list-item-row-r can-show-more" @click="showforeign(item.foreign,item.region)">
                  <svg-icon class="more-foreign" icon-class="more"></svg-icon>
                  <div class="foreign-item" v-for="(val,key) in getArrByJson(item.foreign)" :key="key">
                    <span>
              ...
              <collapse-transition>
                <div class="item-more" v-show="item.show_more&&item.show_more===true">
                  <div class="list-item-row">
                    <div class="list-item-row-l">
                      <div class="justify-4">{{titles.china}}</div>
                    </div>
                    <div class="list-item-row-r" v-html="doEmptyStr(item.china)"></div>
                  </div>
                  <div class="list-item-row">
                    <div class="list-item-row-l">
    ...
    <div>
      <popup v-model="show_filter" class="filter-p-div" position="right"
        ...
        <div class="filter-div">
          ...
          <div class="checker-box">
            <div class="checker-title">{{titles.investment_mode}}：</div>
            <checker v-model="filter['investment_mode[0]']" default-item-class="checker-item"
                     selected-item-class="checker-item-selected">
              <checker-item v-for="val in filter_data.investment_mode_arr" :key="val" :value="val">{{val}}
              </checker-item>
            </checker>
          </div>
          <div class="checker-box">
          ...
          <div class="checker-box">
            ...
            <checker v-model="filter.type" default-item-class="checker-item"
                     selected-item-class="checker-item-selected">
              <checker-item v-for="val in filter_data.type_arr" :key="val" :value="val">{{val}}</checker-item>
            </checker>
          </div>
          <div class="checker-box">
            <div class="checker-title">{{titles.contractural_foreign_investment}}（万美元）：</div>
            <checker v-model="contractural_foreign_investment" default-item-class="checker-item"
                     selected-item-class="checker-item-selected">
              <checker-item :value="{'-1':'','-2':1000}">1000万美元以下</checker-item>
              <checker-item :value="{'-1':1000,'-2':3000}">1000到3000万美元之间</checker-item>
              <checker-item :value="{'-1':1000,'-2':3000}">3000万美元以上</checker-item>
            </checker>
            <div class="filter-input-div">
              <div class="filter-input-title">自定义：</div>
              <input class=" filter-input" v-model="filter['contractural_foreign_investment[-1]']" placeholder="万美元"/>
              <div class="input-time-mind">{{this.time_pacer}}</div>
              <input class=" filter-input" v-model="filter['contractural_foreign_investment[-2]']" placeholder="万美元"/>
            </div>
          </div>
          <div class="checker-box">
            <div class="checker-title">{{titles.investment}}（万美元）：</div>
            ...
            <div class="filter-input-div">
              <div class="filter-input-title">自定义：</div>
              <input class=" filter-input" v-model="filter['investment[-1]']" placeholder="万美元"/>
              <div class="input-time-mind">{{this.time_pacer}}</div>
              <input class=" filter-input" v-model="filter['investment[-2]']" placeholder="万美元"/>
            </div>
          </div>
          <div class="checker-box">
            <div class="checker-title">{{titles.contractural_foreign_investment}} 或 {{titles.investment}}（万美元）：</div>
            <checker v-model="investment_foreign" default-item-class="checker-item"
                     selected-item-class="checker-item-selected">
              <checker-item :value="{'-1':'','-2':1000}">1000万美元以下</checker-item>
              <checker-item :value="{'-1':1000,'-2':3000}">1000到3000万美元之间</checker-item>
              <checker-item :value="{'-1':1000,'-2':3000}">3000万美元以上</checker-item>
            ...
            <div class="filter-input-div">
              <div class="filter-input-title">自定义：</div>
              <input class=" filter-input" v-model="filter['investment_foreign[-1]']" placeholder="万美元"/>
              <div class="input-time-mind">{{this.time_pacer}}</div>
              <input class=" filter-input" v-model="filter['investment_foreign[-2]']" placeholder="万美元"/>
            </div>
          </div>
          <div class="checker-box">
      ...
      <popup v-model="show_foreign">
        <group title="外方信息（外方与所属地区）">
          <cell v-for="(v,k) in foreign_list" :title="v.foreign" :key="k" align-items="flex-start"
                :value="v.region"></cell>
        </group>
      </popup>
      <popup v-model="show_progress">
        ...
        <timeline v-if="progress_list&&progress_list.length>0">
...
<script>
  import { getIndex, getList, getProgressList } from '@/api/progress2'
  ...
  export default {
    ...
    data () {
      return {
        ...
        filter: {
          ...
          'investment[-1]': 0,
          'investment[-2]': 0,
          'contractural_foreign_investment[-1]': 0,
          'contractural_foreign_investment[-2]': 0,
          'investment_foreign[-1]': 0,
          'investment_foreign[-2]': 0,
    ...
    activated () {
      this.$store.dispatch('UpdateTitleBack', { title: '在谈外资项目情况', ShowBack: false })
      this.$store.dispatch('GetHaveUpdated', 'progress2').then(val => {
        // 如果更新了数据
        if (val === true) {
    ...
    computed: {
      ...
      investment_foreign: {
        get: function () {
          return {
            '-1': this.filter['investment_foreign[-1]'],
            '-2': this.filter['investment_foreign[-2]']
          }
        },
        set: function (val) {
          this.filter['investment_foreign[-1]'] = val[-1]
          this.filter['investment_foreign[-2]'] = val[-2]
        }
      },
      contractural_foreign_investment: {
        get: function () {
          return {
            '-1': this.filter['contractural_foreign_investment[-1]'],
            '-2': this.filter['contractural_foreign_investment[-2]']
          }
        },
        set: function (val) {
          this.filter['contractural_foreign_investment[-1]'] = val[-1]
          this.filter['contractural_foreign_investment[-2]'] = val[-2]
        }
      }
    },
    methods: {
      ...
      initData () {
        ...
        getIndex().then(response => {
          ...
          this.filter_data.investment_mode_arr = response.data.investment_mode
      ...
      showforeign (foreign, region) {
        let foreignList = []
        const foreignObj = this.getArrByJson(foreign)
        const regionObj = this.getArrByJson(region)
        for (const i in foreignObj) {
          foreignList.push({
            foreign: foreignObj[i],
            region: regionObj[i] ? regionObj[i] : ''
          })
        }
        this.foreign_list = foreignList
        this.show_foreign = true
      },
      showProgressList (id) {
        this.progress_list = []
